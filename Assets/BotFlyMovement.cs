...
public class BotFlyMovement : MonoBehaviour
{
    ...
    void Update()
    {
        if (canMove)
        {
            if (Mathf.Abs(playerTransform.position.y - transform.position.y) < 25 && Mathf.Abs(playerTransform.position.x - transform.position.x) < 25)
            {
